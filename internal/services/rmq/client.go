...
func (client *Client) connect(addr string) (conn *amqp.Connection, err error) {
...
func (client *Client) Push(data []byte) error {
	...
	if !client.isReady {
		client.m.Unlock()
		return errors.New("[rmq] failed to push: not connected")
	}
	client.m.Unlock()
	for {
		...
		if confirm.Ack {
			client.log.Info("[rmq] push confirmed [%d]", confirm.DeliveryTag)
			return nil
		}
	}
}
